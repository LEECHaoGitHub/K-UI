...
- 每台 VPS 可选择 WARP IPv4、IPv6 或双栈节点出口，并实时下发、验证和回执。
...
服务器卡片使用一个“节点出口”下拉框，提供 `原生出口`、`住宅 IP 代理`、`WARP IPv4`、`WARP IPv6`、`WARP 双栈`。住宅出口自动使用该 VPS 的 `proxy-lite` ACTIVE 隧道、当前住宅端口和 Pages 住宅凭据，无需填写地址、端口、用户名或密码。WARP 只接管该 VPS 上普通代理节点流量，不修改宿主机默认路由，因此不会改变 SSH、Agent 或住宅 OpenVPN 隧道自身的出口。

- IPv4：IPv4 目标走 WARP，IPv6 目标被拒绝，避免原生出口泄漏。
- IPv6：IPv6 目标走 WARP，IPv4 目标被拒绝，避免原生出口泄漏。
- 双栈：IPv4 和 IPv6 目标都走 WARP。
- 住宅 IP 出口与 WARP 出口由统一状态机硬互斥，不能同时启用。
- 首次启用会下载固定版本 `wgcf 2.2.31`，按架构校验 SHA256，并在 VPS 本地注册独立 WARP 身份。
- WARP 私钥只保存在 `/opt/kui/warp.json`，权限为 `600`，不会上传 Pages、D1 或浏览器。
- Agent 会执行 `sing-box check`、重启后服务检查和真实 IPv4/IPv6 WARP 出口测试；失败时恢复上一个已应用模式并回传错误。
...
旧的管理员密码哈希 Agent 鉴权已移除。所有 Agent-only API 只接受该 VPS 自己的 `agent_token`；历史 Agent 必须重新执行面板当前生成的 Full Deploy Command。
